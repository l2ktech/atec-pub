...
# 确保本文件所在目录(demo/)在 sys.path 上，使 `act` 子包在“顶层导入”(server.py)与
# “包内导入”(play_atec_task.py 的 demo.solution -> solution_act)两种方式下都能解析。
...
class AlgSolution:
    ...
    def reset(self, **kwargs):
        ...
        ACT 路径在 episode 内维护了大量状态机/时序缓存（temporal ensembling
        的动作历史、起步零动作计数、回 home 的稳定计数等）。若不在 episode
        之间清空，上一集残留会污染下一集（例如 _home_done 仍为 True 会直接
        跳过回 home 阶段、_action_history 里残留上一集的动作序列被加权平均）。

        本方法把所有“每集状态”重置成与 __init__ 末尾一致的初值：
          - _ts                : 时间步计数 -> 0
          - _action_history    : temporal ensembling 队列 -> 清空
          - _last_action_seq   : 缓存的动作序列 -> None
          - _startup_step      : 起步零动作计数 -> 0
          - _home_stable_steps : 回 home 稳定计数 -> 0
          - _home_done         : 回 home 完成标志 -> False

        约定：
          - 接受任意 **kwargs（平台可能传空 dict 或未知字段），不因此报错；
          - 绝不抛异常（被 try 包裹），返回 None（server.py 忽略返回值）。
        """
        try:
            self._ts = 0
            # deque(maxlen=...) 的 clear() 保留 maxlen；用 clear 而非新建以防
            # 万一队列对象在别处被引用。getattr 兜底：极端情况下属性缺失也不崩。
            hist = getattr(self, "_action_history", None)
            if hist is not None and hasattr(hist, "clear"):
                hist.clear()
            else:
                self._action_history = deque(maxlen=self.num_queries)
            self._last_action_seq = None
            self._startup_step = 0
            self._home_stable_steps = 0
            self._home_done = False
        except Exception as err:  # noqa: BLE001 - reset 绝不能抛出
            print(f"[solution_act] WARN: reset() ignored error ({err})")
        return None
